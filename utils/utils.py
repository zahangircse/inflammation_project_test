...
def thresholding_vectorizing(predicted_mask,actual_masks):
    
    assert (len(predicted_mask.shape)==4 and len(actual_masks.shape)==4)  #4D arrays
    assert (predicted_mask.shape[0]==actual_masks.shape[0])
    assert (predicted_mask.shape[1]==actual_masks.shape[1])
    assert (predicted_mask.shape[2]==actual_masks.shape[2])
    assert (predicted_mask.shape[3]==1 and actual_masks.shape[3]==1)  #check the channel is 1
  
    vec_pred_mask = []
    vec_actual_masks = []
    for img_idx in range(predicted_mask.shape[0]):  #loop over the full images
        ret,y_pred_val = cv2.threshold(predicted_mask[img_idx,:,:,0],0.5,1,cv2.THRESH_BINARY)
        ret,y_true_val = cv2.threshold(actual_masks[img_idx,:,:,0],0.5,1,cv2.THRESH_BINARY) 
        y_hat = np.array(y_pred_val).flatten() 
        y_val = np.array(y_true_val).flatten()
        vec_pred_mask.append(y_hat)
        vec_actual_masks.append(y_val)
                
    vec_pred_mask = np.asarray(vec_pred_mask)
    vec_actual_masks = np.asarray(vec_actual_masks)
    
    return vec_pred_mask, vec_actual_masks
...
# Randomly crop the image to a specific size. For data augmentation
def random_crop_edge(image, label, edge, crop_height, crop_width):
    if (image.shape[0] != label.shape[0]) or (image.shape[1] != label.shape[1]):
        raise Exception('Image and label must have the same dimensions!')
    ...
    if (crop_width <= image.shape[1]) and (crop_height <= image.shape[0]):
        ...
        if len(label.shape) == 3:
            return image[y:y+crop_height, x:x+crop_width, :], label[y:y+crop_height, x:x+crop_width, :], edge[y:y+crop_height, x:x+crop_width, :]
        else:
            return image[y:y+crop_height, x:x+crop_width, :], label[y:y+crop_height, x:x+crop_width], edge[y:y+crop_height, x:x+crop_width]
    else:
        raise Exception('Crop shape (%d, %d) exceeds image dimensions (%d, %d)!' % (crop_height, crop_width, image.shape[0], image.shape[1]))
